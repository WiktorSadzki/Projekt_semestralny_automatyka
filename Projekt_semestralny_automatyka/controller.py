...
class Controller:
    ...
    def predict(self, T, u, Qc, Qg):
        ...
        v_CPU = p.v_min_CPU + (p.v_max_CPU - p.v_min_CPU) * (u_CPU / 100)
        v_GPU = p.v_min_GPU + (p.v_max_GPU - p.v_min_GPU) * (u_GPU / 100)
        v_CASE = p.v_min_case + (p.v_max_case - p.v_min_case) * (u_CASE / 100)
        ...
        m_dot = p.rho_coolant * v_CASE * p.A_fan_case
        Q_vent = m_dot * p.cp_coolant * (T_AIR - p.T_amb)

        # Bilans powietrza w obudowie
        dT_AIR = (Q_conv_CPU + Q_conv_GPU + Q_rad_CPU + Q_rad_GPU - Q_vent - Q_rad_CASE - Q_wall) / p.C_AIR
    ...
    def step(self, T, u_prev, Qc, Qg):
        ...
        def cost(u_flat):
            ...
            for k in range(N):
                u_k = u_seq[k]
                T_sim = self.predict(T_sim, u_k, Qc, Qg)
